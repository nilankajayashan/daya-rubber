<div class="m-0">
    <div class="bg-dark col-12" style="min-height: 100px; border-bottom-right-radius: 50px;">
        <div class="row">
            <h3 class="text-light col-lg-10">Orders</h3>
    ...
    @if(isset($orders) || in_array( 'view-order', json_decode($permissions)))
            ...
            <tbody>
                @foreach($orders as $order)
                    <tr>
                        ...
                        <td>
                            @php $customer = 'Can not find' @endphp
                            @foreach($users as $user)
...
@break
                        ...
                        <td>
                            @if(in_array( 'manage-order', json_decode($permissions)))
                                ...
                                <span class="d-inline-flex">|
                                    <button class="bg-white border-0" data-bs-toggle="modal" data-bs-target="#manageOrder-{{ $order->order_id }}"><i class="fas fa-shipping-fast text-info"></i>Manage</button>
                                </span>
                            @endif
                            @if(in_array( 'edit-order', json_decode($permissions)))
                                <span class="d-inline-flex">|
                                    <button class="bg-white border-0" data-bs-toggle="modal" data-bs-target="#editOrder-{{ $order->order_id }}"><i class="fas fa-edit text-primary"></i>Edit</button>
                                </span>
                            @endif
                    ...
                    @if(in_array( 'edit-order', json_decode($permissions)))
                        {{--            edit order modal--}}
                        <div class="modal fade" id="manageOrder-{{ $order->order_id }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="exampleModalLabel">Manage Order:&nbsp;
                                            <h6><span class="badge @switch($order->order_status)
                                                @case('pending')
                                                    bg-info
@break
