<div class="m-0">
    <div class="bg-dark col-12" style="min-height: 100px; border-bottom-right-radius: 50px;">
        <div class="row">
            <h3 class="text-light col-lg-10">Stocks</h3>
        </div>
    </div>
                        ...
                        @if(in_array( 'edit-product', json_decode($permissions)))
                            <td>
                                <form action="{{ route('update-product-stock') }}" method="post">
                                    ...
                                    <input type="number" class="form-control w-25 d-inline-flex" name="quantity" placeholder="Quantity" required="required" min="1" value="{{ $product->quantity }}">
                                    <button type="submit" class="btn btn-warning d-inline-flex">Update</button>
                                </form>
                            </td>
                        @endif
                    </tr>
