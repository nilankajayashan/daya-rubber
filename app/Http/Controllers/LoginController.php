    ...
    public function loginSubmit(Request $request){
        ...
        if ($validator->fails()){
            if($validator->errors()->has('email')){
                ...
                return redirect()->back()->withErrors( ['password' => 'Please Check you entered password again'])->with(['notify_error' => 'Please Check you entered password again']);
            }
        }
        ...
        try{
            ...
            if($employee != null){
                if( Hash::check($validated['password'], $employee->password)){
                    ...
                    return redirect()->back()->withErrors(['password' => 'Dear '. $validated['email'] .', you entered password is wrong']);
                }
            }else{
                return redirect()->back()->withErrors(['email' => 'You entered email address not registered']);
            }
        }catch (Exception $e){
