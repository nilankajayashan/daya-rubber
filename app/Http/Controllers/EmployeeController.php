...
class EmployeeController extends Controller
{
    public function add(Request $request){
        ...
        try {
            ...
            isset($request->view_supplier)?array_push($permission_list,'view-supplier'):'';
            isset($request->add_supplier)?array_push($permission_list,'add-supplier'):'';
            isset($request->edit_supplier)?array_push($permission_list,'edit-supplier'):'';
            isset($request->delete_supplier)?array_push($permission_list,'delete-supplier'):'';
            isset($request->approve_supplier)?array_push($permission_list,'approve-supplier'):'';
            (isset($request->add_supplier) || isset($request->edit_supplier) || isset($request->delete_supplier) || isset($request->approve_supplier))?(!in_array('view-supplier', $permission_list))?array_push($permission_list,'view-supplier'):'':'';
    ...
    public function changeStatus(Request $request){
        ...
        try{
            ...
            if ($employee->status == 1){
                ...
                $employee->status = 1;
                $employee->approved_by = session()->get('auth_employee')->employee_id;
                $employee->save();
