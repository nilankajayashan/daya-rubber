...
class CartController extends Controller
{
    ...
    public function editView(Request $request){
        ...
        try{
            ...
            if ($cart == null){
                ...
                return view('dashboard', ['state' => 'edit-cart', 'permissions' => $permissions, 'cart' => $full_cart, 'user' => $user,'total' => $total]);
            }
        }catch (Exception $exception){
    ...
    public function removeItemCart(Request $request){
        ...
        if ($validator->fails()){
            if ($validator->errors()->has('user_id')){
                return redirect()->route('dashboard', ['state' => 'carts'])->with(['notify_error' => 'Customer ID is Required to edit cart']);
            }
            if ($validator->errors()->has('product_id')){
                return redirect()->route('dashboard', ['state' => 'carts'])->with(['notify_error' => 'Product ID is Required to edit cart']);
            }
        }
        $validated = $validator->validated();
        try {
            ...
            if($old_cart == null){
                return redirect()->route('dashboard', ['state' => 'carts'])->with(['error' => 'Can not update empty card']);
            }else{
...
//                dd($old_cart_list);
                ...
                $product_name = Product::select('name')
                    ...
                    ->first();
                return redirect()->route('dashboard', ['state' => 'carts'])->with(['success' => $product_name->name.' removed from cart successfully...!' ]);
            }
        }catch (Exception $exception){
            return redirect()->route('dashboard', ['state' => 'carts'])->with(['notify_error' => 'Something Went Wrong'.$exception->getMessage()]);
        }
    }
    ...
    public function updateItemCart(Request $request){
        ...
        if ($validator->fails()){
            ...
            if ($validator->errors()->has('quantity')){
                return redirect()->route('dashboard', ['state' => 'carts'])->with(['notify_error' => 'Product quantity is Required to edit cart']);
            }
        }
        $validated = $validator->validated();
        try {
            ...
            if($product->quantity < $validated['quantity']){
                return redirect()->route('dashboard', ['state' => 'carts'])->with(['notify_warning' => 'This quantity not available in this time | availability: '. $product->quantity]);
            }
            $old_cart = Cart::where('user_id', '=', $validated['user_id'])->first();
            if($old_cart == null){
                return redirect()->route('dashboard', ['state' => 'carts'])->with(['error' => 'Can not update empty card']);
                ...
                $product_name = Product::select('name')
                    ->where('product_id', '=', $validated['product_id'])
                    ->first();
                return redirect()->route('dashboard', ['state' => 'carts'])->with(['success' => $validated['quantity'].' '.$product_name->name.' added to cart successfully...!' ]);
            }
        }catch (Exception $exception){
            return redirect()->route('dashboard', ['state' => 'carts'])->with(['notify_error' => 'Something Went Wrong'.$exception->getMessage()]);
        }
    ...
    public function addToCart(Request $request){
        ...
        if ($validator->fails()){
            if ($validator->errors()->has('user_id')){
                return redirect()->route('dashboard', ['state' => 'carts'])->with(['notify_error' => 'Customer ID is Required to add cart']);
            }
            if ($validator->errors()->has('product_id')){
                return redirect()->route('dashboard', ['state' => 'carts'])->with(['notify_error' => 'Product ID is Required to add cart']);
            }
            if ($validator->errors()->has('quantity')){
                return redirect()->route('dashboard', ['state' => 'carts'])->with(['notify_error' => 'Product quantity is Required to add cart']);
            }
        }
        $validated = $validator->validated();
        try {
